 ...
 * *********************************************************************
 *
 * This file is part of KEEL-software, the Data Mining tool for regression, classification, clustering, pattern mining and so on.
 *
 * Copyright (C) 2004-2010
 *
 * F. Herrera (herrera@decsai.ugr.es) L. S�nchez (luciano@uniovi.es) J. Alcal�-Fdez (jalcala@decsai.ugr.es) S. Garc�a (sglopez@ujaen.es) A. Fern�ndez (alberto.fernandez@ujaen.es) J. Luengo (julianlm@decsai.ugr.es)
 *
...
/**
 * A Java implementation of the ID3 algorithm. This is a binary version where nodes are split by atributtes and values, instead of the generic ID3, which only considers attributes for splitting.
 *
 * @author Crist�bal Romero Morales (UCO)
 * @version 1.0 (30-03-06)
 */
public class REX1 extends Algorithm {
     ...
     *
     */
    List<Double> _entropias;
    List<List<List<Integer>>> _matrizEntropias;
    public REX1(String paramFile) {
        boolean salir = false;
        try {
            ...
            _entropias = new LinkedList();
            _matrizEntropias = new LinkedList();
            
            for (int i=0; i<modelDataset.numAttributes(); ++i){//Dimension de numero de atributos
                if (modelDataset.getAttribute(i) != modelDataset.getClassAttribute()){
                    _matrizEntropias.add(new LinkedList());
                    for (int j=0; j<modelDataset.getAttribute(i).numValues(); ++j){//Dimension de numero de valores
                        _matrizEntropias.get(i).add(new LinkedList());
                        for (int k=0; k<modelDataset.numClasses(); ++k){//Dimension de cada de valor de la clase
                            _matrizEntropias.get(i).get(j).add(new Integer(0));//Rellenamos con 0
                        }
                    }
                }
            }
            int cont = 1;
            for(int i=0; i<_matrizEntropias.size(); ++i){
                for(int j=0; j<_matrizEntropias.get(i).size(); ++j){
                    for(int k=0; k<_matrizEntropias.get(i).get(j).size(); ++k){
                        System.out.println(cont++ + " " + _matrizEntropias.get(i).get(j).get(k));
                    }
            ...
            if (Attributes.hasRealAttributes() || Attributes.hasIntegerAttributes()) {
                System.err.println("REX-1 can only handle nominal attributes.");
                //System.exit(-1);
                salir = true;
            }
            if (!salir) {
                ...
                System.out.println("¡¡¡¡SE VA A EJECUTAR EL PROGRAMA!!!!");
                execute();
                System.out.println("¡¡¡¡SE HA TERMINAO DE EJECUTAR!!!!");
    ...
    public void calculaEntropias(){
        /*for (int i=0; i<modelDataset.numAttributes(); ++i){//Recorremos los atributos
            for(int j=0; j<modelDataset.getAttribute(i).numValues(); ++j){//Recorremos los valores de los atributos
                for (int k=0; k<modelDataset.numItemsets(); ++k){//Recorremos los ejemplos
                    modelDataset.itemset(k).getAttribute(i)
                }
            }
        }*/
    }
...
//	}
     ...
     *
     * @return	True if the attribute and the values are already used to decompose, or false otherwise.
     */
